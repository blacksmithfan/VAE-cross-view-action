...
class siamese:
    ...
    # Create model
    def __init__(self, keep_var, source_videos, target_videos, pair_indi):
        self.x1 = source_videos
        self.x2 = target_videos

        with tf.variable_scope("siamese", reuse=None) as scope:
            self.o1 = Model.slimNet_lstm(self.x1, keep_var)
        with tf.variable_scope("siamese", reuse=True) as scope:
            self.o2 = Model.slimNet_lstm(self.x2, keep_var)
